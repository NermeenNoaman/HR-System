...
namespace HRSystem_Wizer_.Controllers
{
    ...
    {
        private readonly IGenericRepository<LkpJobApplication> _repository;
        private readonly IMapper _mapper;

        public JobApplicationController(IGenericRepository<LkpJobApplication> repository, IMapper mapper)
        {
        ...
        public async Task<IActionResult> GetAll()
        {
            try
            {
                ...
                var dtos = _mapper.Map<IEnumerable<JobApplicationReadDto>>(entities);
                return Ok(dtos);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var dto = _mapper.Map<JobApplicationReadDto>(entity);
                return Ok(dto);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Create([FromBody] JobApplicationCreateDto dto)
        {
            try
            {
                ...
                var createdDto = _mapper.Map<JobApplicationReadDto>(createdEntity);
