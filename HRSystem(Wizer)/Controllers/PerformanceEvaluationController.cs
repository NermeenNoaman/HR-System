...
namespace HRSystem_Wizer_.Controllers
{
    ...
    {
        private readonly IGenericRepository<TPLPerformanceEvaluation> _repository;
        private readonly IMapper _mapper;

        public PerformanceEvaluationController(IGenericRepository<TPLPerformanceEvaluation> repository, IMapper mapper)
        {
        ...
        public async Task<IActionResult> GetAll()
        {
            try
            {
                ...
                var dtos = _mapper.Map<IEnumerable<PerformanceEvaluationReadDto>>(entities);
                return Ok(dtos);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var dto = _mapper.Map<PerformanceEvaluationReadDto>(entity);
                return Ok(dto);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Create([FromBody] PerformanceEvaluationCreateDto dto)
        {
            try
            {
                ...
                var createdDto = _mapper.Map<PerformanceEvaluationReadDto>(createdEntity);
